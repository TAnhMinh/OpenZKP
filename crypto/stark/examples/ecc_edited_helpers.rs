...
use zkp_u256::U256;
use zkp_primefield::{FieldElement, One, SquareInline, Zero};
...
pub const SHIFT_POINT: (FieldElement, FieldElement) = (
    field_element!("049ee3eba8c1600700ee1b87eb599f16716b0b1022947733551fde4050ca6804"),
    field_element!("03ca0cfe4b3bc6ddf346d49d06ea0ed34e621062c0e056c1d0405d266e10268a")
    );

pub const GENERATOR: (FieldElement, FieldElement) = (
    field_element!("033840300bf6cec10429bf5184041c7b51a9bf65d4403deac9019623cf0273dd"),
    field_element!("05a0e71610f55329fbd89a97cf4b33ad0939e3442869bbe7569d0da34235308a")
    );
// pub const SHIFT_POINT: (FieldElement, FieldElement) = (
...
const A : FieldElement = field_element!("01");
...
pub fn scalar_mult(trace: &mut TraceTable, point: (FieldElement, FieldElement), scalar: &U256, start: usize, offset: usize, neg: bool)  {
    ...
    for i in 0..256 {
        ...
        info!("Trace {:?}, 0: {:?}", i, trace[(start + i, offset + 0)]);
        info!("Trace {:?}, 1: {:?}", i, trace[(start + i, offset + 1)]);
        info!("Trace {:?}, 2: {:?}", i, trace[(start + i, offset + 2)]);
        info!("Trace {:?}, 3: {:?}", i, trace[(start + i, offset + 3)]);
        info!("Trace {:?}, 4: {:?}", i, trace[(start + i, offset + 4)]);
        info!(" ");
    }
}
...
mod test {
    ...
    fn scalar_mult_in_table() {
        ...
        // let scalar_c = ScalarFieldElement::from(u256h!(
        ...
        let expected_affine = Affine::Point{x: p.0.clone(), y: p.1.clone()} * scalar_c;
